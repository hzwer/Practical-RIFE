...
class IFNet(nn.Module):
    ...
    def forward(self, x, scale_list=[4, 2, 1], scale=1.0, training=False):
        ...
        for i in range(3):
            f0, m0 = block[i](torch.cat((warped_img0, warped_img1, mask), 1), flow, scale=scale_list[i])
            f1, m1 = block[i](torch.cat((warped_img1, warped_img0, -mask), 1), torch.cat((flow[:, 2:4], flow[:, :2]), 1), scale=scale_list[i])
